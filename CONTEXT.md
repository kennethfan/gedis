...
| **已兼容子集（现状）** | v1 已实现：String, Hash, List, Set（含 TTL/HEXPIRE、PSYNC 主从、LRU、INFO/SLOWLOG/Prometheus）；M1 已交付：ZSet 全量（含 LEX/STORE/阻塞）+ Geo 套壳 + Bitmap 全量 + HLL 全量（dense-only）+ SCAN/TYPE 补齐；M2 已交付：Stream 全量（含消费组/PEL/阻塞读）；M4 已交付：事务（MULTI/EXEC/WATCH）；M5 已交付：Pub/Sub（含 pattern 订阅）；M6 已交付：Lua（EVAL/EVALSHA/SCRIPT + cjson + SCRIPT KILL + 超时可配 + 脚本内禁用命令 + 编译措辞对齐 + 沙箱只读化）；未实现：Cluster/Sentinel、Lua 脚本内三库（bit 位运算/cmsgpack/struct，非 Bitmap 命令）、cjson 配置函数 |
